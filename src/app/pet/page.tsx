  ...
  return (
    <main className="relative w-full h-screen h-dvh overflow-hidden">
      ...
      <div className="absolute bottom-8 left-0 right-0 flex flex-col items-center z-10">
          ...
          className={`relative w-16 h-16 rounded-full transition-all duration-200 ease-out cursor-pointer
            ...
            ${isRecording 
              ...
              : isTranscribing || isChatProcessing
                ? 'bg-white/20 backdrop-blur-sm opacity-50 cursor-wait'
        ...
        >
          ...
          {(isTranscribing || isChatProcessing) && (
            <span className="absolute inset-0 flex items-center justify-center">
              <svg
                className="w-8 h-8 animate-spin text-white/60"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
              >
        ...
        <p className="text-center mt-3 text-sm font-light text-white/70">
          {isRecording 
            ? 'Listening...' 
            : isTranscribing
              ? 'Processing...'
      ...
      {(isTranscribing || isChatProcessing) && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10">
          <div className="px-4 py-2 bg-white/30 backdrop-blur-sm rounded-full">
            <p className="text-sm text-white">
              {isTranscribing ? 'Processing speech...' : 'Thinking...'}
            </p>
          </div>
        </div>
      )}
